...
public class GenerateSQL {
    ...
    final static String DELETE_EXISTING_TABLE = "drop table if exists ";
    final static String SKOLEM_TABLE_NAME = SpicyEngineConstants.WORK_SCHEMA_NAME + ".skolemTable";
    final static String SKOLEM_TABLE_COLUMN_ID = "id";
    final static String SKOLEM_TABLE_COLUMN_SKOLEM = "skolem";
    final static String SKOLEM_VIEW_NAME = SpicyEngineConstants.WORK_SCHEMA_NAME + ".skolemString";
    ...
    private static JepToPostgresConverter postgresConv = new JepToPostgresConverter();
    private GeneratePLSQLfunctions functionEvaluation = new GeneratePLSQLfunctions();
    private static int scenarioNo;
    public static boolean newSequence;
    
    public String generateSQL(MappingTask mappingTask, int scenarioNo) {
        ...
        result.append("-- This script was automatically generated by the MIPMap mapping tool.\n\n");
        ...
        return result.toString().replaceAll(SpicyEngineConstants.DOUBLE_QUOTES, "\"");        
    ...
    public static String getSourceSchemaName(int chainingStep) {
        if (chainingStep == CHAINING_FIRST_STEP || chainingStep == CHAINING_NO_CHAINING) {
            return SpicyEngineConstants.SOURCE_SCHEMA_NAME + scenarioNo;
        }
    ...
    public static String getTargetSchemaName(int chainingStep) {
        ...
        if (chainingStep == CHAINING_LAST_STEP || chainingStep == CHAINING_NO_CHAINING) {
            return SpicyEngineConstants.TARGET_SCHEMA_NAME + scenarioNo;
        }
        throw new IllegalArgumentException("Wrong value for chainingStep parameter: " + chainingStep);
    }
...
//////////////////////  SQL VIEW NAMES
    ...
    public static String tgdFinalSQLName(FORule tgd) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + "TARGET_VALUES_" + sqlNameForTgdWithNoSchemaName(tgd);
    }
    
    private static String sqlNameForTgdWithNoSchemaName(ConstantFORule tgd) {
        return replaceSpecialCharacters(tgd.getId().toLowerCase());
    }
    
    public static String tgdFinalSQLName(ConstantFORule tgd) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + "TARGET_VALUES_" + sqlNameForTgdWithNoSchemaName(tgd);
    }

    public static String finalSqlNameAfterExchange(SetAlias variable) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + "EXCHANGE_RESULT_FOR_" + variable.getBindingPathExpression().getLastStep();
    }
    ...
    public static String attributeNameWithVariable(VariablePathExpression attributePath) {
        return RELATION_ALIAS_PREFIX + attributePath.getStartingVariable().toShortString() + "." 
                //giannisk
                //put column names in double quotes, BUT for now use a fixed value which will be replaced at the end
                //so that the double quotes are not affected during the function conversion procces
                + SpicyEngineConstants.DOUBLE_QUOTES + attributePath.getLastStep()+ SpicyEngineConstants.DOUBLE_QUOTES;
    }
    ...
    private static String sqlString(Expression expression) {
        ...
        for (int i=0; i<attributePaths.size();i++) { 
            ...
            String attributeName = extractAttributeNameFromExpression(attributePath);
            result = result.replaceAll(attributePath.toString(), attributeName);
        }
    ...
    public static String sqlNameForViewWithIntersection(ComplexQueryWithNegations query) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + query.getId();
    }

    public static String sqlNameForPositiveView(ComplexQueryWithNegations query) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + POSITIVE_QUERY + query.getId();
    }

    public static String sqlNameForRule(FORule tgd) {
        return SpicyEngineConstants.WORK_SCHEMA_NAME + "." + sqlNameForTgdWithNoSchemaName(tgd);
    }
    ...
    private String getDropCreateWorkSchemaScript(){
        StringBuilder result = new StringBuilder();
        result.append("drop schema if exists ").append(SpicyEngineConstants.WORK_SCHEMA_NAME).append(" cascade;\n");
        result.append("create schema ").append(SpicyEngineConstants.WORK_SCHEMA_NAME).append(";\n");
        return result.toString();
    }
    ...
    public String getDeleteTablesScript(MappingTask mappingTask) {
        ...
        for (PathExpression pathExpression : setPaths) {
            ...
            if (!(node instanceof SetCloneNode)) {
                result.append("delete from ").append(SpicyEngineConstants.TARGET_SCHEMA_NAME).append(scenarioNo).append(".\"").append(pathExpression.getLastStep()).append("\";\n");
            }
        }
        return result.toString();
    }
    ...
    public static VariablePathExpression findSourcePath(VariableCorrespondence variableCorrespondence) {
        if (variableCorrespondence!=null) {
...
//                if (i != variables.size() - 1) result.append(", ");
    ...
    private static String generateWhereClauseForIntersection(ComplexConjunctiveQuery view) {        
        ...
        for (int i = 0; i < allSelectionConditions.size(); i++) {
            ...
            result.append(DOUBLE_INDENT).append(GenerateSQL.functionSql(condition.getCondition(),true).replaceAll("\"", "\'"));
    ...
    private static String generateSQLStringForAdditionalJoinConditionSourcePath(VariableJoinCondition joinCondition, List<VariableCorrespondence> correspondences, String negationViewName) {
        ...
        for (int i = 0; i < joinCondition.getFromPaths().size(); i++) {
            ...
            VariablePathExpression fromSourcePath = findSourcePath(findCorrespondenceFromTargetPathWithSameId(fromPath, correspondences));
            if (fromSourcePath == null) {
                fromSourcePath = fromPath;
            }
            VariablePathExpression toSourcePath = findSourcePath(findCorrespondenceFromTargetPathWithSameId(toPath, correspondences));
...
//        }
    ...
    public static String projectionOnTargetValuesFromCorrespondences(List<VariableCorrespondence> correspondences, String fromClause, String whereClause) {
        StringBuilder result = new StringBuilder();
        result.append(INDENT).append("select distinct \n");
        List <String> alreadyCorrespondences = new ArrayList <String>();
        for (int i = 0; i < correspondences.size(); i++) {
            VariableCorrespondence correspondence = correspondences.get(i);
            //giannisk 
            //so as not to take into account constant values
            if(!correspondence.isConstant()){
                //giannisk, so as not to take into account variables already selected
                if (!alreadyCorrespondences.contains(correspondence.getFirstSourcePath().toString())){
                    if (correspondence.getSourcePaths() != null) {
                        ...
                        alreadyCorrespondences.add(correspondence.getFirstSourcePath().toString());
                    } 
                    else {
                    ...
                    /*if ((i != correspondences.size() - 1)
                            //giannisk 
                            //so as not to take into account constant values
                            && (!correspondences.get(i+1).isConstant())) {
                        result.append(",\n");
                    }*/
                }
            }
