...
class NestedLoopJoinOperator implements IJoinOperator {
    ...
    private INode joinInstances(INode firstInstanceRoot, INode secondInstanceRoot, VariableJoinCondition joinCondition) {
        ...
        for (INode firstTuple : firstInstanceRoot.getChildren()) {
            for (INode secondTuple : secondInstanceRoot.getChildren()) {
                ...
                //if it is a match string join
                if (joinCondition.isMatchString()){
                    joincheck = stringMatchListsForJoin(firstTupleValues, secondTupleValues);
                }
                //else an equality join
                else{
                    joincheck = equalListsForJoin(firstTupleValues, secondTupleValues);
                }
                if (joincheck) {
    ...
    @SuppressWarnings("unchecked")
    private boolean stringMatchListsForJoin(List list1, List list2) {
        if (isEmpty(list1) || isEmpty(list2)) {
            return false;
        }
        boolean stringMatch = false;
        //list1 contains the "fromValues"
        //list2 contains the "toValues"
        for (Object str1 : list1) {
            for (Object str2 : list2){
                if(str1.toString().toLowerCase().contains(str2.toString().toLowerCase())
                      || str2.toString().toLowerCase().contains(str1.toString().toLowerCase())){
                    stringMatch = true;
                }
            }
        }
