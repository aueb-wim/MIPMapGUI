...
public class CheckExpressions {
    ...
    public Expression checkSourceValueAndGenerateExpression(ISourceValue sourceValue) throws ExpressionSyntaxException {
        ...
        } else if (sourceValueString.equals(SpicyEngineConstants.SOURCEVALUE_DATETIME_FUNCTION)) {
            if (!(sourceValue instanceof DatetimeFunction)) {
                throw new ExpressionSyntaxException("Source value of the wrong type (must be a DatetimeFunction): " + sourceValue);
            }
        }
        else { //constant
