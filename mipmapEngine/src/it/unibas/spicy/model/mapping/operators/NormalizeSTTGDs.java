        ...
        if (tgd.getTargetView().getVariables().size() == 1) {
            ...
            if (connectedGroups.size() == 1) {
                ...
                for (ConnectedVariables connectedVariables : connectedGroups) {
                    ...
                    removeCorrespondences(tgd, newTGD);
...
//                    }
    ...
    //giannisk
    public boolean checkCoverageForSource(FORule tgd) {
        boolean flag = false;
        for (VariableCorrespondence vc: tgd.getCoveredCorrespondences()){
            if (!vc.isConstant()){
                flag = true;
                break;
            }
