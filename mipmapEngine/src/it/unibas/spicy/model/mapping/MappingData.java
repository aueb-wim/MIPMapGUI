...
import it.unibas.spicy.model.mapping.operators.MinimizeConstantTGDs;
...
public class MappingData {
    ...
    //giannisk
    public List<ConstantFORule> getConstantSTTgds() {
        if (this.constantSTTgds == null) {
            this.constantSTTgds = new ArrayList<ConstantFORule>(getCandidateConstantSTTgds());
            /*if (this.mappingTask.getConfig().rewriteOverlaps()) {
                this.stTgds = new ArrayList<ConstantFORule>(getOverlapSTTgds());
            }*/
        }
        return this.constantSTTgds;
    }
    
    //giannisk
    public List<ConstantFORule> getCandidateConstantSTTgds() {
        if (this.candidateconstantSTTgds == null) {
            if (this.mappingTask.getLoadedConstantTgds() == null) {
                this.candidateconstantSTTgds = new MinimizeConstantTGDs().generateAndMinimizeTGDs(mappingTask);
            } else {
                this.candidateconstantSTTgds = mappingTask.getLoadedConstantTgds();
            }
            /*if (this.mappingTask.getConfig().rewriteTgdsForSourceNulls()) {
                RewriteTgdsToHandleSourceNulls rewriter = new RewriteTgdsToHandleSourceNulls();
                this.candidateSTTgds = rewriter.rewrite(this.candidateSTTgds, this.mappingTask.getSourceProxy());
        ...
        return candidateconstantSTTgds;
    }
...
//        return getExpansionsMoreCompactOrderMap().getExpansions();
    ...
    //giannisk
    public void verifySolution(IDataSourceProxy solution) {
        GenerateSolution generator = new GenerateSolution();
        generator.verifySolution(solution, mappingTask);
    }
...
//            result.append("Generators: \n").append(tgd.getGenerators(mappingTask));
        ...
        //giannisk
        if(!this.getConstantSTTgds().isEmpty()){
            result.append("================ Final Constant FO Rules (in logical form) ==================\n");
            for (ConstantFORule tgd : getConstantSTTgds()) {
                result.append(tgd.toLogicalString(mappingTask)).append("\n");
            } 
        }
        return result.toString();
