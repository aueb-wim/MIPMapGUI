...
//            ex.printStackTrace();
     ...
     *                        SAVE
     *  ///////////////////////////////////////////////////////// */
    public void saveMappingTask(MappingTask mappingTask, String filePath, boolean rulesBeforeRewriting) throws DAOException {
        ...
        if (rulesBeforeRewriting){
            //List<FORule> candidateRules = renamer.renameAliasesInRules(mappingTask.getMappingData().getSTTgds());
            List<FORule> candidateRules = mappingTask.getMappingData().getSTTgds();
            if (!candidateRules.isEmpty()){
                result.append("SOURCE TO TARGET TGDs (BEFORE REWRITING):\n");
                for (FORule tgd : candidateRules) {
        ...
        List<FORule> rewrittenRules = mappingTask.getMappingData().getRewrittenRules();
        EngineConfiguration conf = mappingTask.getConfig();
        if(!rewrittenRules.isEmpty()){
        ...
        //giannisk
        List<ConstantFORule> constantFORules = mappingTask.getMappingData().getConstantSTTgds();
        if (!constantFORules.isEmpty()){
            result.append("TARGET CONSTANT TGDs:\n");
            for (ConstantFORule tgd : constantFORules) {
                result.append(tgd.toSaveString("", mappingTask)).append("\n");
            }
        }
