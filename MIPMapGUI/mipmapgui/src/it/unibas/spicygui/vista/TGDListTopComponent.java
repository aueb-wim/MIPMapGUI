...
public final class TGDListTopComponent extends TopComponent {
    ...
    public void createTGD() throws IllegalMappingTaskException {
        ...
        for (ConstantFORule constantFORule : constantFORules) {
            TGDTabbedPane tGDTabbedPane = new TGDTabbedPane(constantFORule, scenario, mappingTask);
            this.tabbedPaneTGDs.addTab("TGD - Constant FORule " + i++, tGDTabbedPane);
        }
    }
    ...
    private boolean tgdsEquals() {
        MappingTask mappingTask = scenario.getMappingTask();
        List<FORule> foRules = mappingTask.getMappingData().getSTTgds();
        List<ConstantFORule> constFoRules = mappingTask.getMappingData().getConstantSTTgds();
        if (!((foRules.size()+constFoRules.size()) == this.tabbedPaneTGDs.getTabCount())) {
            return false;
        }
        ...
        //giannisk
        for (int i = foRules.size(); i < constFoRules.size()+foRules.size(); i++) {
            ConstantFORule constFoRule = constFoRules.get(i- foRules.size());
            TGDTabbedPane tGDTabbedPane = (TGDTabbedPane) this.tabbedPaneTGDs.getComponentAt(i);
            if (tGDTabbedPane == null) {
                return true;
            }
            if (!constFoRule.equals(tGDTabbedPane.getConstantFORule())) {
                return false;
            }
        }
        return true;
