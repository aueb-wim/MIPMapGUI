...
//        jLayeredPane.createIntermediateZonePopUp();
    ...
    public void creaWidgetCorrespondences() {
        ...
        ConstantFORule constFoRule = jLayeredPane.getMappingTaskTopComponent().getScenario().getSelectedConstantFORule();
...
//                creaCandidateCorrespondence(valueCorrespondence, mappingTask);
        ...
        //giannisk        
        else if (constFoRule != null && constFoRule.getCoveredCorrespondences().size() > 0) {
            for (int i = 0; i < constFoRule.getCoveredCorrespondences().size(); i++) {
                VariableCorrespondence variableCorrespondence = constFoRule.getCoveredCorrespondences().get(i);                
                creaSourceValue(variableCorrespondence);                               
            }
        }
    }
...
//    }
    ...
    public void creaWidgetIconForSelectionCondition() {
        //giannisk
        //commented
        //used to put 'false' value on selection conditions when Generate was pressed
