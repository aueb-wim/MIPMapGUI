...
public class MyPopupProviderJoinCondition implements PopupMenuProvider, ActionListener {
    ...
    private final String TOGGLE_MATCH_STRING = "toggle match string join";
    ...
    private void createPopupMenu() {
        ...
        Binding binding3 = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ, joinCondition, ELProperty.create("${matchString}"), item4, BeanProperty.create("selected"));
    ...
    private void toggleMatchString(){
        JoinConstraint joinConstraint = (JoinConstraint) this.constraintsLayer.getChildConstraint(rootWidget);
        joinConstraint.getDataSource().setMatchStringForJoin(joinCondition, !joinCondition.isMatchString());////
        joinConstraint.changeMatchString(joinCondition.isMatchString());////
        updateMappingTask();
        StatusDisplayer.getDefault().setStatusText("Match String Join: " + joinCondition.isMatchString());
    }
