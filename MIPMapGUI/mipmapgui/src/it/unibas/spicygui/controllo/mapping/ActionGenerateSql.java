...
public class ActionGenerateSql extends CallableSystemAction implements Observer {
    ...
    public ActionGenerateSql() {
        executeInjection();
        this.putValue(SHORT_DESCRIPTION, NbBundle.getMessage(Costanti.class, Costanti.ACTION_GENERATE_SQL_TOOLTIP));
...
//        } else if (stato.equals(LastActionBean.CLOSE) || (stato.equals(LastActionBean.NO_SCENARIO_SELECTED))) {
    ...
    private void execute(Scenario scenario) {
        try {
            SqlTopComponent generateSqlTopComponent = scenario.getSqlTopComponent();
            if (generateSqlTopComponent == null) {
                generateSqlTopComponent = new SqlTopComponent(scenario);
                scenario.setSqlTopComponent(generateSqlTopComponent);
            }
            ...
            generateSqlTopComponent.clear();
            generateSqlTopComponent.createQueryTabs();
            generateSqlTopComponent.openAtTabPosition(10);
            generateSqlTopComponent.requestActive();
        } catch (Exception ex) {
    ...
    public String getName() {
        return NbBundle.getMessage(Costanti.class, Costanti.ACTION_GENERATE_SQL);
    }
